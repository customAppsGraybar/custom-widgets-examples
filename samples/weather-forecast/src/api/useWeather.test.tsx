...
describe('useWeather', () => {
    ...
    it('should return weather data', async () => {
        ...
        const { result, waitFor } = renderHook(() => useWeather({ key: "foo", lat: 44, lon: 42 }), { wrapper });

        await waitFor(() => {
            return result.current.isSuccess;
        });

        expect(mockAxios).toHaveBeenCalledWith("//api.openweathermap.org/data/2.5/onecall", { params: { appid: "foo", lat: 44, lon: 42, lang: 'en', units: 'standard' } })
        expect(result.current.data?.current).toMatchObject({
            date: "Thu 20 May", "description": "scattered clouds", "humidity": 90,
            "temperature": {
                "current": "282",
            },
            "wind": "1"
    ...
    it('should return an error, when the data processing goes wrong', async () => {
        ...
        expect(result.current.error).toEqual(new TypeError("Cannot read property 'map' of undefined"));
        expect(log).toHaveBeenCalledTimes(1);
    })
});
